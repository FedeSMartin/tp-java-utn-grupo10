	...
	public void parsearArchivo() {
		...
		try {
			...
			listaDePronosticos = new CsvToBeanBuilder(new FileReader(this.rutaArchivoPronosticos))
					...
					// con esta configuración podemos skipear la primera línea de nuestro archivo
					// CSV
					.withSkipLines(1)

					// con esta configuración podemos elegir cual es el caracter que vamos a usar
					// para delimitar
					.withSeparator(';')

					// es necesario definir el tipo de dato que va a generar el objeto que estamos
					// queriendo parsear a partir del CSV
					.withType(ArchivoPronosticos.class).build().parse();
